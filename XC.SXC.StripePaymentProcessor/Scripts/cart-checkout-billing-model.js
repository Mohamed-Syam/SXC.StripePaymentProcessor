...
function AchPaymentViewModel(billingViewModel, card){
	var self = this;
	self.paymentProcessed = ko.observable(false);
	    self.billingViewModel = ko.observable(billingViewModel);
		self.achButtonMessage = ko.observable('Use ACH Payment');
 
	self.processACHPayment = function(obj,event){
		event.preventDefault();
			self.billingViewModel().paymentProcessing(true);

			$.ajax({ cache: false,
					url: "/api/cxa/xccheckout/SetAchPaymentMethod",
					type: "POST",
					...
					self.billingViewModel().paymentProcessing(false);

					self.billingViewModel().achStripePaymentProcessed(true);
					self.billingViewModel().achStripePaymentProcessed.valueHasMutated();
					}
					else{
					...
					console.log("error submitting Stripe ACH payment", data);
					self.billingViewModel().paymentProcessing(false);

				});
		
	};
	self.achButtonShow = ko.computed(function(){
		if(self.paymentProcessed()){
			$('#submitAchPayment').prop("disabled", true);
			return self.achButtonMessage('ACH Payment Processed');
		}
		return self.achButtonMessage('ACH Payment');
	});
...
// -----[BILLING DATA MODEL]-----
function BillingDataViewModel(checkoutViewModel, creditCardPaymentViewModel) {
    var self = this;
	...
	self.checkoutViewModel = checkoutViewModel;
	self.creditCardPaymentViewModel = ko.observable(creditCardPaymentViewModel);
    ...
    self.achPayment = ko.validatedObservable(new AchPaymentViewModel(self));
    ...
    self.achStripePaymentProcessed = ko.observable(false);
    ...
    self.validateEmailBilling = ko.observable(false);
    ...
    self.selectedBillingAddress.subscribe(function (id) {
	    ...
	    self.billingAddress.subscribe(function (id) {
			console.log('billing address changed');
        });
    
	 self.achStripePaymentProcessed.subscribe(function (id) {
         $(".to-confirm-button").prop("disabled", false);
      });
    ...
    self.enableToConfirm = ko.computed({
        read: function () {
            ...
            if (self.giftCardPayment().isAdded()) {
			 ...
			 if(self.achPayment().paymentProcessed()){
    ...
    self.validateEmailBilling = ko.computed(function() {
        return self.billingEmail.isValid() && self.billingConfirmEmail.isValid() && self.billingAddress.isValid();
    });
    
    self.isEmailFilled = function(event){
        
         if(self.billingEmail.isValid() && self.billingConfirmEmail.isValid()) {
        //     self.billingAddress();
             return true;
         }
        else{
            ...
            // self.billingEmail('`');
            // self.billingEmail('');
            // self.billingConfirmEmail('`');
            // self.billingConfirmEmail('');
            return false;
            // self.billingEmail.showAllMessages();
            // self.billingConfirmEmail.showAllMessages();
        }
    }
    ...
    function initModelData(data) {
        ...
        self.isShipAll((data.Cart != null && data.Cart.Shipments != null && data.Cart.Shipments.length == 1) ? true : false);
